...
if (!BOT_TOKEN || BOT_TOKEN === "DEIN_BOT_TOKEN") { console.error("❌ BOT TOKEN FEHLT!"); process.exit(1); }
...
let d = {
    users: {}, chats: {}, links: {}, tracker: {}, counter: {}, warte: {},
    gepostet: [], seasonStart: Date.now(), seasonGewinner: [],
    dailyXP: {}, weeklyXP: {}, dailyReset: null, weeklyReset: null,
    bonusLinks: {}, wochenGewinnspiel: { aktiv: true, gewinner: [], letzteAuslosung: null },
    warteNachricht: {}, dmNachrichten: {}, instaWarte: {},
    missionen: {}, wochenMissionen: {}, missionQueue: {}, missionQueueVerarbeitet: null,
    missionAuswertungErledigt: {}, gesternDailyXP: {}, badgeTracker: {}, m1Streak: {},
    backupDatum: null, _lastEvents: {},
...
function laden() {
    try {
        if (fs.existsSync(DATA_FILE)) {
            ...
            if (!d.dailyXP) d.dailyXP = {}; if (!d.weeklyXP) d.weeklyXP = {};
            if (!d.bonusLinks) d.bonusLinks = {}; if (!d.missionen) d.missionen = {};
            if (!d.wochenMissionen) d.wochenMissionen = {}; if (!d.warteNachricht) d.warteNachricht = {};
            if (!d.dmNachrichten) d.dmNachrichten = {}; if (!d.instaWarte) d.instaWarte = {};
            if (!d.wochenGewinnspiel) d.wochenGewinnspiel = { aktiv: true, gewinner: [], letzteAuslosung: null };
            if (!d.missionQueue) d.missionQueue = {}; if (!d.gesternDailyXP) d.gesternDailyXP = {};
            if (!d.badgeTracker) d.badgeTracker = {}; if (!d.m1Streak) d.m1Streak = {};
            if (!d.missionAuswertungErledigt) d.missionAuswertungErledigt = {};
            if (!d._lastEvents) d._lastEvents = {};
            if (!d.xpEvent) d.xpEvent = { aktiv: false, multiplier: 1, start: null, end: null, announced: false };
...
function speichern() {
    ...
    finally { isSaving = false; if (savePending) { savePending = false; setTimeout(speichern, 100); } }
}
let saveTimer = null;
function speichernDebounced() { if (saveTimer) return; saveTimer = setTimeout(() => { saveTimer = null; speichern(); }, 2000); }
...
async function checkInstagramForAllUsers(bot) {
    ...
    for (const [uid, u] of Object.entries(d.users)) {
        ...
        } catch (e) { console.log('❌ DM fehlgeschlagen bei', uid); }
    }
    speichern();
}
...
async function backup() {
    try {
        ...
        const s = Object.assign({}, d); s.links = {};
        for (const [k, v] of Object.entries(d.links)) s.links[k] = Object.assign({}, v, { likes: Array.from(v.likes) });
        fs.writeFileSync(backupFile, JSON.stringify(s, null, 2));
        d.backupDatum = heute; console.log('✅ Backup:', backupFile);
...
function xpAdd(uid, menge, name) {
    ...
    let finalXP = menge;
    if (d.xpEvent && d.xpEvent.aktiv && d.xpEvent.multiplier > 1) finalXP = Math.round(menge * d.xpEvent.multiplier);
...
function user(uid, name) {
    if (!d.users[uid]) {
        d.users[uid] = { name: name || '', username: null, instagram: null, xp: 0, level: 1,
            warnings: 0, started: false, links: 0, likes: 0, role: '🆕 New', lastDaily: null, totalLikes: 0, chats: [] };
    }
...
function chat(cid, obj) {
    if (!d.chats[cid]) d.chats[cid] = { id: cid, type: (obj && obj.type) || 'unknown', title: (obj && (obj.title || obj.first_name)) || 'Unbekannt', msgs: 0 };
...
function istInstagramLink(text) { if (!text) return false; const t = text.toLowerCase(); return t.includes('instagram.com') || t.includes('instagr.am'); }
...
function istSperrzeit() {
    const jetzt = new Date(); const tag = jetzt.getDay(); const h = jetzt.getHours();
...
function getMission(uid) {
    const heute = new Date().toDateString();
    if (!d.missionen[uid] || d.missionen[uid].date !== heute) d.missionen[uid] = { date: heute, likesGegeben: 0, m1: false, m2: false, m3: false };
...
async function checkMissionen(uid, name) {
    ...
    if (!mission.m1 && mission.likesGegeben >= 5) {
        mission.m1 = true; d.missionQueue[uid].m1Pending = true;
...
async function missionenAuswerten() {
    const heute = new Date().toDateString(); const jetzt12 = heute + '_12';
    ...
    for (const [uid, queue] of Object.entries(d.missionQueue)) {
        ...
        const wMission = getWochenMission(uid); const gestern = queue.date; let meldungen = [];
        ...
        if (queue.m1Pending) {
            xpAdd(uid, 5, name); meldungen.push('✅ *Mission 1!*\n5 Links geliked → +5 XP');
        ...
        if (gesamtGestern > 0 && prozentGestern >= 0.8) {
            xpAdd(uid, 5, name); meldungen.push('✅ *Mission 2!*\n' + Math.round(prozentGestern * 100) + '% geliked → +5 XP');
            if (wMission.letzterTag !== gestern) { wMission.m2Tage++; if (wMission.m2Tage >= 7) { xpAdd(uid, 15, name); meldungen.push('🏆 *Wochen-M2!* +15 XP'); wMission.m2Tage = 0; } }
        }
        if (gesamtGestern > 0 && gelikedGestern === gesamtGestern) {
            xpAdd(uid, 5, name); meldungen.push('✅ *Mission 3!*\nAlle Links geliked → +5 XP');
            if (wMission.letzterTag !== gestern) { wMission.m3Tage++; if (wMission.m3Tage >= 7) { xpAdd(uid, 20, name); meldungen.push('🏆 *Wochen-M3!* +20 XP'); wMission.m3Tage = 0; } }
        }
        ...
        if (queue.m1Pending) {
            d.m1Streak[uid].count++; d.m1Streak[uid].letzterTag = gestern;
            if (d.m1Streak[uid].count >= 5 && d.users[uid] && d.users[uid].warnings > 0) {
                d.users[uid].warnings--; d.m1Streak[uid].count = 0;
        ...
        if (meldungen.length > 0 && d.users[uid]) {
            const u = d.users[uid]; const nb = xpBisNaechstesBadge(u.xp);
    ...
    const nurHeute = {}; if (d.missionAuswertungErledigt[jetzt12]) nurHeute[jetzt12] = true;
    d.missionAuswertungErledigt = nurHeute; speichern();
}
...
async function weeklyRankingDM() {
    ...
    for (const [uid] of Object.entries(d.users)) {
        ...
        const rank = sorted.findIndex(([id]) => id === uid); if (rank === -1) continue;
        const xp = d.weeklyXP[uid] || 0; const u = d.users[uid];
...
bot.start(async (ctx) => {
    const uid = ctx.from.id; const u = user(uid, ctx.from.first_name); u.started = true;
...
bot.command('help', async (ctx) => {
    const uid = ctx.from.id; const u = user(uid, ctx.from.first_name);
...
bot.command('missionen', async (ctx) => {
    ...
    const u = user(uid, ctx.from.first_name); const mission = getMission(uid); const wMission = getWochenMission(uid);
    const heute = new Date().toDateString();
...
bot.command('profile', async (ctx) => {
    ...
    const rank = sorted.findIndex(x => x[0] == uid) + 1; const bonusL = d.bonusLinks[uid] || 0;
    await ctx.reply(
...
bot.command('ranking', async (ctx) => {
    ...
    const b = ['🥇', '🥈', '🥉']; let text = '🏆 *GESAMT RANKING*\n\n';
...
bot.command('dailyranking', async (ctx) => {
    ...
    const b = ['🥇', '🥈', '🥉']; let text = '📅 *TAGES RANKING*\n\n';
...
bot.command('weeklyranking', async (ctx) => {
    ...
    const b = ['🥇', '🥈', '🥉']; let text = '📆 *WOCHEN RANKING*\n\n';
    sorted.forEach(([uid, xp], i) => { text += (b[i] || (i + 1) + '.') + ' ' + d.users[uid].role + ' *' + d.users[uid].name + '*\n   ⭐ ' + xp + ' XP\n\n'; });
    await ctx.reply(text, { parse_mode: 'Markdown' });
});
...
bot.command('daily', async (ctx) => {
    ...
    const u = user(uid, ctx.from.first_name); const jetzt = Date.now(); const h24 = 86400000;
    ...
    const bonus = Math.floor(Math.random() * 20) + 10; u.lastDaily = jetzt;
    xpAddMitDaily(uid, bonus, ctx.from.first_name); speichern();
...
bot.command('dashboard', async (ctx) => {
    ...
    await ctx.reply('📊 Admin Dashboard:', {
        reply_markup: { inline_keyboard: [[{ text: '🚀 Dashboard öffnen', url: 'https://site--telegram-bot--899dydmn7d7v.code.run/dashboard' }]] }
    });
...
bot.command('dm', async (ctx) => {
    ...
    let ok = 0, err = 0; await ctx.reply('📨 Sende...');
    for (const [uid, u] of Object.entries(d.users)) {
...
bot.command('extralink', async (ctx) => {
    ...
    if (!d.bonusLinks[uid]) d.bonusLinks[uid] = 0; d.bonusLinks[uid] += 1; speichern();
...
bot.command('testmissionauswertung', async (ctx) => {
    ...
    if (d.missionAuswertungErledigt) d.missionAuswertungErledigt = {};
    await missionenAuswerten(); await ctx.reply('✅ Auswertung!');
});
...
bot.on('new_chat_members', async (ctx) => {
    for (const m of ctx.message.new_chat_members) {
        ...
        d.warte[m.id] = ctx.chat.id; user(m.id, m.first_name);
        const info = await ctx.telegram.getMe();
...
bot.on('message', async (ctx) => {
    try {
        if (istPrivat(ctx.chat.type) && d.instaWarte[ctx.from.id]) {
            const text = ctx.message.text; if (!text) return;
            const clean = text.replace('@', '').trim();
            const u = user(ctx.from.id, ctx.from.first_name); u.instagram = clean;
            delete d.instaWarte[ctx.from.id]; speichern();
            await ctx.reply('✅ Instagram gespeichert: @' + clean); return;
        }
        ...
        if (!hatLink(text)) {
            if (ctx.chat.id === -1003800312818) {
                ...
                if (!istAdminMsg) {
                    try {
                        await ctx.forwardMessage(-1003906557227); await ctx.deleteMessage();
        ...
        if (!u.started) {
            ...
            d.warteNachricht[uid] = { chatId: ctx.chat.id, msgId: warteMsg.message_id }; speichern(); return;
        }
        if (istSperrzeit() && !admin) {
            try { await ctx.deleteMessage(); } catch (e) {}
            const msg = await ctx.reply('🚫 Keine Links von Sonntag 20:00 bis Montag 06:00!');
            setTimeout(async () => { try { await ctx.telegram.deleteMessage(ctx.chat.id, msg.message_id); } catch (e) {} }, 15000); return;
        }
        const url = linkUrl(text); const urlNorm = normalisiereUrl(url);
        ...
        if (!admin && d.tracker[uid] === heute) {
            if (hatBonusLink(uid)) { bonusLinkNutzen(uid); await ctx.reply('🎁 Bonus Link genutzt!'); }
            else if (badgeBonusLinks(u.xp) > 0 && (!d.badgeTracker[uid] || d.badgeTracker[uid] !== heute)) { d.badgeTracker[uid] = heute; await ctx.reply('🏅 Erfahrener Extra Link genutzt!'); }
            else {
                try { await ctx.deleteMessage(); } catch (e) {}
                d.counter[uid]++;
                if (u.warnings >= 5) {
        ...
        const msgId = ctx.message.message_id; const istInsta = istInstagramLink(text);
        if (istInsta) {
            try { await ctx.deleteMessage(); } catch (e) {}
            ...
            d.links[msgId] = { chat_id: ctx.chat.id, user_id: uid, user_name: ctx.from.first_name, text: text, likes: new Set(), likerNames: {}, counter_msg_id: botMsg.message_id, timestamp: Date.now() };
            if (!istAdminId(uid)) {
                try {
            ...
            if (linkKeys.length > 500) { const oldest = linkKeys.sort((a, b) => d.links[a].timestamp - d.links[b].timestamp)[0]; delete d.links[oldest]; }
            await sendeLinkAnAlle(d.links[msgId]);
        } else {
            d.links[msgId] = { chat_id: ctx.chat.id, user_id: uid, user_name: ctx.from.first_name, text: text, likes: new Set(), counter_msg_id: msgId, timestamp: Date.now() };
        }
        speichern();
...
bot.action(/^like_(\d+)$/, async (ctx) => {
    const msgId = parseInt(ctx.match[1]); const uid = ctx.from.id; const likeKey = msgId + '_' + uid;
    if (likeInProgress.has(likeKey)) { try { await ctx.answerCbQuery(); } catch (e) {} return; }
    likeInProgress.add(likeKey); setTimeout(() => likeInProgress.delete(likeKey), 5000);
    try {
        ...
        lnk.likes.add(uid); lnk.likerNames[uid] = { name: ctx.from.first_name, insta: d.users[uid]?.instagram || null };
        const anz = lnk.likes.size; const poster = user(lnk.user_id, lnk.user_name); poster.totalLikes++;
        ...
        const liker = user(uid, ctx.from.first_name); const nb = xpBisNaechstesBadge(liker.xp);
        const eventBonus = d.xpEvent && d.xpEvent.aktiv && d.xpEvent.multiplier > 1 ? ` (+${Math.round((d.xpEvent.multiplier - 1) * 100)}% Event)` : '';
...
bot.action('remind_insta', async (ctx) => {
    ...
    for (const [uid, u] of Object.entries(d.users)) {
        ...
        try { await bot.telegram.sendMessage(Number(uid), '📸 Bitte sende mir deinen Instagram Namen.\n\n(z.B. max123)', { reply_markup: { inline_keyboard: [[{ text: '📸 Instagram eingeben', callback_data: 'set_insta' }]] } }); count++; await new Promise(r => setTimeout(r, 100)); } catch (e) {}
    }
...
bot.action('set_insta', async (ctx) => {
    try {
        const uid = ctx.from.id; if (!d.instaWarte) d.instaWarte = {};
        d.instaWarte[uid] = true; speichern();
...
async function sendeLinkAnAlle(linkData) {
    ...
    for (let i = 0; i < empfaenger.length; i += 10) {
        ...
        const results = await Promise.allSettled(batch.map(([uid]) => sendeDM(uid, '📢 Neuer Booster-Link\n\n👤 ' + linkData.user_name + '\n\n🔗 ' + linkData.text + '\n\nBitte liken! 👍', { reply_markup: { inline_keyboard: [[{ text: '👉 Zum Beitrag', url: linkUrl2 }]] } })));
        results.forEach((result, idx) => { if (result.status === 'fulfilled' && result.value) d.dmNachrichten[msgKey][batch[idx][0]] = result.value.message_id; });
...
async function dailyRankingAbschluss() {
    ...
    for (let i = 0; i < Math.min(3, sorted.length); i++) {
        const [uid, xp] = sorted[i]; const u = d.users[uid]; const b = bel[i];
    ...
    d.dailyXP = {}; d.tracker = {}; d.counter = {}; d.badgeTracker = {}; d.dailyReset = Date.now(); speichern();
}
...
async function gesternRankingPosten() {
    ...
    if (!sorted.length) return;
    const b = ['🥇', '🥈', '🥉']; let text = '🌅 *VORTAGS RANKING*\n\n';
...
async function wochenGewinnspiel() {
    ...
    const gwUid = teilnehmer[Math.floor(Math.random() * teilnehmer.length)]; const gw = d.users[gwUid];
    if (!d.bonusLinks[gwUid]) d.bonusLinks[gwUid] = 0; d.bonusLinks[gwUid] += 1;
    ...
    d.weeklyXP = {}; d.weeklyReset = Date.now(); speichern(); await weeklyRankingDM();
}
...
async function abendM1Warnung() {
    ...
    for (const [uid, u] of Object.entries(d.users)) {
        ...
        const m = d.missionen[uid]; if (m && m.date === heute && m.m1) continue;
...
async function zeitCheck() {
    try {
        const jetzt = new Date(); const h = jetzt.getHours(); const m = jetzt.getMinutes();
        const wochentag = jetzt.getDay(); const eventKey = h + ':' + m + ':' + jetzt.toDateString();
        if (!d._lastEvents) d._lastEvents = {};
        const gruppen = Object.values(d.chats).filter(c => istGruppe(c.type));
        ...
        if (h === 6 && m === 0 && d._lastEvents['regeln'] !== eventKey) {
            ...
            gruppen.forEach(g => { bot.telegram.sendMessage(g.id, '📜 *Regeln*\n\n1️⃣ 1 Link pro Tag\n2️⃣ Keine Duplikate\n3️⃣ Bot starten\n4️⃣ 5 Warns = Ban\n5️⃣ Respekt\n\n👍 Jeden Link liken & kommentieren!\n🔍 Tägliche Kontrollen', { parse_mode: 'Markdown' }).catch(() => {}); });
        ...
        for (const [k, l] of Object.entries(d.links)) {
            if (Date.now() - l.timestamp > zweiTage) {
                ...
                if (d.dmNachrichten && d.dmNachrichten[mk]) { for (const [uid2, dmId] of Object.entries(d.dmNachrichten[mk])) bot.telegram.deleteMessage(Number(uid2), dmId).catch(() => {}); delete d.dmNachrichten[mk]; }
                const lu = linkUrl(l.text); if (lu) { const idx = d.gepostet.indexOf(lu); if (idx !== -1) d.gepostet.splice(idx, 1); }
                delete d.links[k];
            }
        }
        ...
        if (d.xpEvent && d.xpEvent.start && d.xpEvent.end) {
            const now = Date.now(); const percent = Math.round((d.xpEvent.multiplier - 1) * 100);
            ...
            if (!d.xpEvent.announced && now >= d.xpEvent.start - 1800000 && now < d.xpEvent.start) {
                ...
                gruppen.forEach(g => { bot.telegram.sendMessage(g.id, `📢 *XP EVENT kommt!*\n\n🔥 +${percent}% XP\n⏱️ Dauer: ${dauerMin} Minuten`, { parse_mode: 'Markdown' }).catch(() => {}); });
                speichern();
            }
            if (!d.xpEvent.aktiv && now >= d.xpEvent.start && now <= d.xpEvent.end) {
                d.xpEvent.aktiv = true;
                gruppen.forEach(g => { bot.telegram.sendMessage(g.id, `🚀 *XP EVENT GESTARTET!*\n\n🔥 +${percent}% XP\n⏱️ Dauer: ${dauerMin} Minuten\n🕒 Ende: ${endZeit}`, { parse_mode: 'Markdown' }).catch(() => {}); });
                speichern();
            }
            if (d.xpEvent.aktiv && now > d.xpEvent.end) {
                d.xpEvent.aktiv = false;
                gruppen.forEach(g => { bot.telegram.sendMessage(g.id, `⏱️ *XP EVENT BEENDET*\n\n🔥 Event vorbei\n📉 XP wieder normal`, { parse_mode: 'Markdown' }).catch(() => {}); });
                speichern();
            }
        }
...
// ================================================================
// ERSETZE NUR DIESEN BLOCK IN DEINER bot.js:
// Von: app.get('/dashboard', (req, res) => {
// Bis: das zugehörige }); (vor app.get('/reset-user'))
// ================================================================
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 20px 22px; transition: border-color .2s, transform .2s; position: relative; overflow: hidden; }
  ...
  .stat-icon.c-green  { background: var(--green-bg); }
  .stat-icon.c-blue   { background: var(--blue-bg); }
  .stat-icon.c-amber  { background: var(--amber-bg); }
  .stat-icon.c-red    { background: var(--red-bg); }
  ...
  .event-card.active { border-color: rgba(16,185,129,.4); box-shadow: 0 0 0 1px rgba(16,185,129,.15), var(--shadow); }
  ...
  .event-title { font-size: 14px; font-weight: 600; display: flex; align-items: center; gap: 8px; }
  .badge-pill { font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 20px; letter-spacing: .5px; }
  ...
  .mission-count { font-size: 36px; font-weight: 800; line-height: 1; letter-spacing: -1px; }
  ...
  .link-rank { width: 22px; font-size: 16px; flex-shrink: 0; text-align: center; }
  ...
  .search-input { width: 100%; background: var(--surface2); border: 1px solid var(--border2); color: var(--text); border-radius: var(--radius-sm); padding: 10px 14px; font-size: 13px; outline: none; transition: border-color .2s; }
  ...
  .action-link:hover { filter: brightness(1.2); transform: translateY(-1px); }
  ...
  .insta-warn { background: var(--amber-bg); border: 1px solid rgba(245,158,11,.3); border-radius: var(--radius-sm); padding: 12px 16px; font-size: 13px; color: var(--amber); margin-bottom: 14px; display: flex; align-items: center; gap: 8px; }
  ...
  <div class="event-card ${evtAktiv ? 'active' : ''} mb-28">
    ...
    <div class="event-status-row">
      <div class="event-stat">
        <div class="event-stat-label">Status</div>
        <div class="event-stat-value" style="color:${evtAktiv ? 'var(--green)' : 'var(--muted)'}">${evtAktiv ? 'Läuft' : 'Gestoppt'}</div>
      </div>
      <div class="event-stat">
        <div class="event-stat-label">Bonus</div>
        <div class="event-stat-value" style="color:var(--amber)">${evtPct > 0 ? '+' + evtPct + '%' : '—'}</div>
      </div>
      <div class="event-stat">
        <div class="event-stat-label">Start</div>
        <div class="event-stat-value">${evtStartStr}</div>
      </div>
      <div class="event-stat">
        <div class="event-stat-label">Ende</div>
        <div class="event-stat-value">${evtEndStr}</div>
      </div>
    </div>
  </div>
    ...
    <div class="card-body">
      <div class="mission-grid">
        <div class="mission-item">
          <div class="mission-id">MISSION 1</div>
          <div class="mission-count m1">${m1c}</div>
          <div class="mission-sub">User erfüllt</div>
        </div>
        <div class="mission-item">
          <div class="mission-id">MISSION 2</div>
          <div class="mission-count m2">${m2c}</div>
          <div class="mission-sub">User erfüllt</div>
        </div>
        <div class="mission-item">
          <div class="mission-id">MISSION 3</div>
          <div class="mission-count m3">${m3c}</div>
          <div class="mission-sub">User erfüllt</div>
        </div>
      </div>
    </div>
        ...
        <div class="user-row">
          ...
          <div class="user-info">
            ...
            <div class="user-meta">
              ...
              ${mData ? `&nbsp;·&nbsp; M:${mData.m1 ? '✓' : '✗'}${mData.m2 ? '✓' : '✗'}${mData.m3 ? '✓' : '✗'}` : ''}
            </div>
          </div>
          <div>
...
app.get('/create-xp-event', (req, res) => {
    ...
    let startTime = startType === 'custom' && req.query.startCustom ? new Date(req.query.startCustom).getTime() : Date.now();
